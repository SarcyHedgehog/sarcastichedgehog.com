...
export class ArenaView {
  ...
  startIntro(){clearTimeout(this.introTimer);const token=Symbol("intro");this.intro={started:performance.now(),duration:4800,token};this.introTimer=setTimeout(()=>{if(this.intro?.token===token)this.resetCamera()},5200);this.camera.position.copy(this.cameraHome);this.camera.fov=68;this.camera.updateProjectionMatrix();this.keyLight.intensity=.05;this.rimLight.intensity=0;for(const light of this.showLights)light.intensity=0}

  resetCamera(){clearTimeout(this.introTimer);this.introTimer=null;this.intro=null;this.camera.position.copy(this.cameraHome);this.camera.fov=48;this.camera.updateProjectionMatrix();this.camera.up.set(0,1,0);this.camera.lookAt(0,0,0);this.camera.updateMatrixWorld(true);this.keyLight.intensity=3.8;this.rimLight.intensity=13;for(const light of this.showLights)light.intensity=0}
